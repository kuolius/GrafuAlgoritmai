   ...
   public class KruskalioMJM
    {
        ...
        public KruskalioMJM(Screen screen)
        {
            this.screen = screen;
        }
        ...
        public void Execute()
        {
            foreach (Vertix vx in vertix)
            {
                ...
                vx.S = new List<int> { vertix.IndexOf(vx) };
            ...
            foreach(Edge eg in edge)
            {
                if(!Equal(vertix[eg.x].S,vertix[eg.y].S))
                {
                    eg.pi = true;
                    vertix[eg.x].pi = true;
                    vertix[eg.y].pi = true;

                    List<int> temp= Join(vertix[eg.x].S, vertix[eg.y].S);
        ...
        public void Draw()
        {
            ...
            foreach (Edge eg in edge)
            {
                ...
                if(vertix[eg.x].coord.X>= vertix[eg.y].coord.X && vertix[eg.x].coord.Y<= vertix[eg.y].coord.Y || vertix[eg.x].coord.X <= vertix[eg.y].coord.X && vertix[eg.x].coord.Y >= vertix[eg.y].coord.Y)
                    spriteBatch.DrawString(mainFont, eg.mass.ToString(), new Vector2(vertix[eg.x].coord.X / 2 + vertix[eg.y].coord.X / 2, vertix[eg.x].coord.Y / 2 + vertix[eg.y].coord.Y / 2+5), Color.Black);
                else
                    spriteBatch.DrawString(mainFont, eg.mass.ToString(), new Vector2(vertix[eg.x].coord.X / 2 + vertix[eg.y].coord.X / 2, vertix[eg.x].coord.Y / 2 + vertix[eg.y].coord.Y / 2-20), Color.Black);
